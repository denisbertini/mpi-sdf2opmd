...
int sdf_io(int argc, char *argv[]) {
  ...
  if (mpi_rank == 0 ){
    if ( info_name.length() == 0 ){
      std::cout << std::endl;
      std::cout << "sdf2opmd_1d: file to process: " << sdf_file.c_str() << std::endl;
      ...
      std::cout << std::endl;      
      std::cout << "sdf2opmd_1d: dry run:  reading SDF blocks info: " << info_name << " from file: " << sdf_file << std::endl;      
      std::cout << std::endl;
    }
  }
  ...
  if (!derived_list.empty()) {
	...
	series.iterations[1].meshes[blockid.c_str()][MeshRecordComponent::SCALAR];
      ...
      Extent global_extent = {field_x.global_sx};
	...
	cout << "Prepared a Field Dataset of size " << dataset.extent[0] << "x"
      ...
      Offset chunk_offset = {field_x.l_dx};
      Extent chunk_extent = {field_x.l_sx};
