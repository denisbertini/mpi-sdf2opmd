...
CONTAINS
  ...
  subroutine read_field_data_r8(filename, block_id, field_data, grid_x, name, &
    ...
    real(num), dimension(:), allocatable, intent(out) :: grid_x
    ...
    IF (ndims /= c_ndims) THEN
      IF (rank == 0) THEN
        PRINT*, '*** ERROR ***'
        PRINT*, 'Wrong number of dimensions ndims: ', ndims, ' c_ndims: ', c_ndims
      END IF
      STOP
    END IF
    ...
    ALLOCATE(field_data(local_sizes(1)))
    field_data = 0.0_num
    ...
    CALL sdf_read_srl_plain_mesh(sdf_handle, grid_x)
  ...
  SUBROUTINE read_particle_data(filename, species_name, particle_data, npart_global, npart_proc, start, columns)
    ...
    ! First column will be 1-D grid data
    ...
    CHARACTER(LEN=c_id_length) :: code_name, block_id, mesh_id, species_id, found_mesh_id
   ...
   IF (rank == 0) PRINT*, 'Found  species_name: ', species_name, ' npart: ', npart, &
    ...
    ! Rewind again once for columns info
 ...
 subroutine read_derived_variables(filename)
    ...
    logical :: restart_flag, found
    integer :: iblock
    character(len=c_id_length) :: tmpblock_id, name
