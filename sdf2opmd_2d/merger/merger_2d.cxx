...
namespace converter
{
  namespace dim_2
  {
    ...
    Merger_2d::Merger_2d(int rank, int size)
      : m_verbosity(0)			   
      , m_min_npart_pcell(4)
    ...
    void Merger_2d::merge(part &pp, const int* x_bins, const int* p_bins){    
	...
	for(int j=0;j<x_bins[1]; j++){
	  int cell_index = j * x_bins[0] + i;

	  if ((m_verbosity>1) && (0 == m_mpi_rank)){ 
	    std::cout << "linear cell index i: " << i << " j: " << j  << " : "
		      << cell_index << std::endl;
	    int i_y = (cell_index/x_bins[0]);
	    int i_x = (cell_index % x_bins[0]);
	  ...
	  for (int k=0; k<n_part;k++){
		...
		cells_indexes[cell_index].push_back(k);
	  }//!k
	}//!j
      }//!i
    ...
    void  Merger_2d::p_cartesian(part &pp, const int* x_bins, const int* p_bins,
