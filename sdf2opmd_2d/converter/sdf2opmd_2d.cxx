...
namespace converter
{
  ...
  {  
	...
	if ( info_name.length() == 0 ){
	  ...
	  std::cout << std::endl;
	  std::cout << "sdf2opmd_2d: dry run:  reading SDF blocks info: " << info_name
      ...
      for(std::string sdf_file : common_sdf_files)
	{
	  ...
	  for (size_t k=0; k < field_list.size(); k++) {
	    ...
	    if (0 == mpi_rank){
	      ...
	      std::cout << " gridx min: " << field_x.gridx[0] << " gridx max: " << field_x.gridx[field_x.l_gridx-1] << std::endl; 
	      std::cout << " gridy min: " << field_x.gridy[0] << " gridy max: " << field_x.gridy[field_x.l_gridy-1] << std::endl;       
	      std::cout << std::endl;
	    }
	    ...
	    // Create a 2D mesh for Fields E, B, J 
	    ...
	    if (flag && meta_fields[0]){
	      ...
	      double gy_spacing = (field_x.gridy[field_x.l_gridy-1]-field_x.gridy[0])/field_x.global_sx;	    
	      mesh.setGridSpacing(std::vector<double>{gx_spacing, gy_spacing})
		.setGridGlobalOffset({field_x.gridx[0], field_x.gridy[0]});	  
	  ...
	  for (size_t i_spec =0; i_spec < species_list.size(); i_spec++) {
	    ...
	    */
	    ...
	    int red_index=0;
	    for ( const auto en : PartVars2D::All ){
	      const PartVars2D::Type val_en = static_cast<PartVars2D::Type>(en);
	      switch (val_en){
	      case PartVars2D::x:
		if (0 == mpi_rank)		
		...
		break;            
	      default:
		assert( ! "Invalid PartVars2D enum value" );
		break;
	      }
	    }
...
using namespace converter::dim_2;
